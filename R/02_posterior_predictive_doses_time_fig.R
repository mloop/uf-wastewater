...
readRDS("../output/02_posterior_predictive_doses.rds") %>%
  ...
  summarise(median_mean_consumption = Hmisc::wtd.quantile(consumption_per_1000_over_locations, weights = location_weight, probs = 0.5, na.rm = FALSE),
            low_mean_consumption = Hmisc::wtd.quantile(consumption_per_1000_over_locations, weights = location_weight, probs = 0.25, na.rm = FALSE),
            high_mean_consumption = Hmisc::wtd.quantile(consumption_per_1000_over_locations, weights = location_weight, probs = 0.75, na.rm = FALSE),
  ) %>%  
